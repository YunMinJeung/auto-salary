...
<section class="store-hero">
  <div>
    ...
    <div class="hero-badges">
      <span class="hbadge"><i class="bi bi-calendar3"></i><?= date('Y년 n월 j일') ?></span>
      <span class="hbadge"><i class="bi bi-building"></i><?= ($settings['employee_count_type'] ?? '') === 'over5' ? '5인 이상 사업장' : '5인 미만 사업장' ?></span>
      <span class="hbadge"><i class="bi bi-currency-won"></i>최저시급 <?= number_format((int)($settings['minimum_wage'] ?? 0)) ?>원</span>
    </div>
  </div>
  <div class="hero-actions">
    <a href="<?= url('qr') ?>" class="btn btn-secondary btn-sm">
      <i class="bi bi-qr-code"></i>QR 출퇴근
    </a>
    <a href="<?= url('work_logs', 'create') ?>" class="btn btn-primary btn-sm">
      <i class="bi bi-plus"></i>근무 기록 추가
    </a>
  </div>
</section>
...
<div class="summary-grid">
  ...
  <a href="<?= url('members') ?>" class="summary-card">
    <div class="summary-icon icon-people"><i class="bi bi-people-fill"></i></div>
  ...
  <a href="<?= url('attendance') ?>" class="summary-card">
    <div class="summary-icon icon-clock"><i class="bi bi-door-open-fill"></i></div>
  ...
  <a href="<?= url('labor_risk') ?>" class="summary-card">
  ...
  <a href="<?= url('payroll') ?>" class="summary-card">
    <div class="summary-icon icon-payroll"><i class="bi bi-cash-coin"></i></div>
    <div class="summary-value" style="font-size:1.1rem;letter-spacing:0;color:var(--primary);font-weight:800;">
      바로가기 →
    </div>
...
<div class="grid-2">
  ...
  <div class="card">
    <div class="card-header">
      <span><i class="bi bi-clock-fill lead-i"></i>오늘 출퇴근 현황</span>
  ...
  <div class="card">
    <div class="card-header">
      <i class="bi bi-bell-fill lead-i"></i>확인 필요한 항목
    </div>
    <div class="card-body">
      <?php if ($needCheck === 0): ?>
      <div class="empty-state">
...
<div class="grid-2 even">
  ...
  <div class="card">
    <div class="card-header">
      <span><i class="bi bi-people-fill lead-i"></i>재직 중 직원</span>
      <a href="<?= url('members', 'add') ?>" class="btn btn-primary btn-sm">
        <i class="bi bi-plus"></i>직원 추가
      </a>
    </div>
    <div class="card-body">
      ...
      <div class="row-item">
        <div class="avatar"><?= h($initial) ?></div>
        <div class="flex-1">
          <div class="row-name"><?= h($emp['name'] ?? '') ?></div>
          <div class="row-sub">시급 <?= number_format((int)($emp['hourly_wage'] ?? 0)) ?>원</div>
        </div>
        <div class="row-right">
  ...
  <div class="card">
    <div class="card-header">
      <span><i class="bi bi-clock-history lead-i"></i>최근 근무 기록</span>
    ...
    <div class="card-body">
      ...
      ?>
      <div class="row-item">
        <div class="wdot <?= $isAbsent ? 'absent' : '' ?>"></div>
        <div class="flex-1">
          <div>
            <span class="row-name"><?= h($log['employee_name'] ?? '') ?></span>
            <span class="wdate"><?= h($log['work_date'] ?? '') ?>(<?= dayOfWeekKo($log['work_date'] ?? '') ?>)</span>
          </div>
          <div class="wtime">
            <?php if ($isAbsent): ?>
            ...
            <?php else: ?>
              ...
              <?php if ($durationStr): ?>
                <span class="wdur"><?= $durationStr ?></span>
              <?php endif; ?>
            <?php endif; ?>
          </div>
        </div>
...
<div class="grid-charts">
  ...
  <div class="card">
    <div class="card-header"><i class="bi bi-bar-chart-fill lead-i"></i>월별 인건비 추이</div>
    <div style="padding:1rem 1.25rem"><canvas id="chartMonthly" height="160"></canvas></div>
  </div>
  <?php endif; ?>
  <?php if (!empty($weeklyHours)): ?>
  <div class="card">
    <div class="card-header"><i class="bi bi-person-clock lead-i"></i>이번 주 직원별 근무시간</div>
    <div style="padding:1rem 1.25rem"><canvas id="chartWeekly" height="160"></canvas></div>
  </div>
  <?php endif; ?>
</div>
<script>
    ...
    new Chart(ctx1, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: 'rgba(15,118,110,.5)',
          borderColor: '#0F766E',
          borderWidth: 1.5,
          borderRadius: 5
        }]
    ...
    new Chart(ctx2, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: 'rgba(241,148,180,.5)',
          borderColor: '#F194B4',
          borderWidth: 1.5,
          borderRadius: 5
        }]
